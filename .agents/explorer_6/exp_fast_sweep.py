...
# Pre-fetch and prepare data once
data = {}
...
def simulate_portfolio(
    ...
    for sym in ['BTC/USDT', 'ETH/USDT', 'SOL/USDT']:
        ...
        for start in range(WARMUP, len(df) - STEP, STEP):
            ...
            def objective(trial):
                ...
                # Ensure spacing * tp >= sl to pass geometry guard
    ...
    # Combined balance series per step
    ...
    print("\n" + "=" * 65)
    print(f"PORTFOLIO RESULTS (Leverage={leverage}x, ER={er_limits})")
    print("=" * 65)
    for sym, r in symbol_results.items():
        print(f"{sym:10s}: Bal=${r['final_balance']:7.2f} | PnL=${r['pnl']:+7.2f} | PF={r['pf']:5.2f} | MaxDD={r['max_dd']:5.2f}% | WFO={r['wfo_acc']} | Trades={r['trades']}")
    print("-" * 65)
    print(f"TOTAL PORTFOLIO: Initial=${total_initial:.2f} -> Final=${total_final:.2f}")
    print(f"PnL=${portfolio_pnl:+7.2f} USD | ROI={portfolio_roi:+.2f}% | PF={port_pf:.2f} | MaxDD={port_max_dd:.2f}%")
    print("=" * 65)
    return portfolio_roi, port_pf, port_max_dd

if __name__ == '__main__':
    print("Test 1: Leverage 16x, ER BTC 0.20 / ETH 0.20 / SOL 0.22, Spacing (0.50, 1.80), Risk (0.04, 0.12)")
    simulate_portfolio(
        er_limits={'BTC/USDT': 0.20, 'ETH/USDT': 0.20, 'SOL/USDT': 0.22},
        spacing_range=(0.50, 1.80),
        tp_range=(1.30, 3.20),
