...
def run_experiment(leverage=16, risk_min=0.03, risk_max=0.10, 
                   min_oos_trades=2, min_oos_pf=1.10, max_oos_dd=0.20,
                   n_trials=300, spacing_min=0.3, spacing_max=1.5,
                   tp_min=1.2, tp_max=3.5, sl_min=0.5, sl_max=1.8):
    ...
    for sym in SYMBOLS:
        ...
        for start in range(WARMUP, len(df) - STEP, STEP):
            ...
            new_balance, trades = run_live_replay(
                ...
                0.35, 0.80, 0.0008, 0.0024, 25.0, 0.0002,
    ...
    print("=" * 60)
    print(f"Leverage: {leverage}x | Risk: [{risk_min}, {risk_max}] | OOS Trades: >={min_oos_trades} | PF: >={min_oos_pf} | DD: <={max_oos_dd}")
    print(f"Portfolio ROI: {portfolio_roi:+.2f}% | Max DD: {port_dd:.2f}% | PF: {portfolio_pf:.2f} | PnL: ${portfolio_pnl:+.2f}")
    print("=" * 60)
