...
def fetch_data(sym, timeframe, limit=10000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_DAILYWFO_FIX_{limit}.csv"
...
def run_backtest(df, start_idx, end_idx, initial_capital, params):
    ...
    while i < n - max_hold:
        ...
        # Eliminamos el filtro estricto EMA200 para permitir operaciones agresivas intradia
        is_l = (rsi[i] < rsi_os)
        is_s = (rsi[i] > rsi_ob)
...
def main():
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        def objective(trial):
            params = {
                ...
                'rsi_os': trial.suggest_int('rsi_os', 15, 45),
                'risk_pct': trial.suggest_float('risk_pct', MAX_RISK*0.2, MAX_RISK),
                'atr_sl_mult': trial.suggest_float('atr_sl_mult', 0.5, 3.0),
                'ts_act_mult': trial.suggest_float('ts_act_mult', 0.5, 3.0),
                'ts_dist_mult': trial.suggest_float('ts_dist_mult', 0.1, 1.5)
            }
    ...
    ax.set_title(f"Optimización Diaria Dinámica (30 Días) | Semanal OOS: {weekly_avg_return*100:.2f}%", fontsize=16, color='white')
    ...
    with open(f"{artifact_dir}/daily_wfo_results.md", "w", encoding='utf-8') as f:
        f.write(f"# Resultados Optimización Diaria Dinámica\n")
