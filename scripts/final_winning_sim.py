...
def main():
    print("EJECUTANDO SIMULACION DEL SANTO GRIAL (PARAMETROS GLOBALES OPTIMOS)")
    ...
    for sym in symbols:
        ...
        all_dfs[sym] = create_labels(df_prep, sl_mult, tp_mult)
    ...
    for sym in symbols:
        ...
        ml = xgb.XGBClassifier(n_estimators=100, max_depth=max_depth, learning_rate=lr, n_jobs=-1, random_state=42)
        ms = xgb.XGBClassifier(n_estimators=100, max_depth=max_depth, learning_rate=lr, n_jobs=-1, random_state=42)
        
        print(f"[{sym}] Entrenando XGBoost con Global Optimals...")
    ...
    print(f"\\n=================================================")
    print(f"SIMULACION OPTIMA 90 DIAS COMPLETADA.")
    ...
    ax.set_title("Evolucion de Cuenta 3 Meses (THE HOLY GRAIL - OPTIMA GLOBAL)", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/holy_grail_equity.png", dpi=100, bbox_inches='tight')
