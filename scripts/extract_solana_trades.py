...
params = {'max_depth': 4, 'learning_rate': 0.07805649487352909, 'reg_alpha': 4.35863575668277, 'reg_lambda': 4.446290706296152, 'confidence': 0.6883034798215051, 'sl_mult': 1.5962207601651157, 'tp_mult': 4.867826554945944, 'risk_pct': 0.3127042793669176}
sym = 'SOL/USDT'
cache_dir = 'data'
if not os.path.exists(cache_dir):
    cache_dir = '../data'

artifact_path = r"C:\Users\Manu\.gemini\antigravity\brain\783f53a5-fa72-4405-a613-4b976ee52762\historial_solana.md"
...
def extract_trades():
    ...
    df_raw = pd.read_csv(cache_file, index_col='timestamp', parse_dates=True).tail(10000)
    ...
    md_content = "# 📈 Bitácora de Operaciones: Solana (SOL/USDT)\\n\\n"
    md_content += "Este reporte detalla cada operación ejecutada por el modelo de IA operando con el **100% de tu capital ($250)** durante los últimos 20 días en datos Out-of-Sample.\\n\\n"
    
    md_content += "| # | Tipo | Entrada | Salida | Razón | Resultado | Capital |\\n"
    md_content += "|---|---|---|---|---|---|---|\\n"
    ...
    while i < n - max_hold:
        ...
        if is_l or is_s:
            ...
            for j in range(1, max_hold + 1):
                ...
                if es_long:
                    ...
                    if curr_l <= sl_price: 
                        ...
                        exit_idx = i+j
                        exit_reason = "Trailing Stop" if ts_activated else "Stop Loss"
                        break
                    if curr_h >= tp_price: 
                        salida = tp_price
                        exit_idx = i+j
            ...
            t_type = "🟢 LONG" if es_long else "🔴 SHORT"
            t_in = f"{timestamps[i].strftime('%d %b %H:%M')}<br>${entrada:.2f}"
            t_out = f"{timestamps[exit_idx].strftime('%d %b %H:%M')}<br>${salida:.2f}"
            
            res_val = f"+${ganancia_usd:.2f}" if ganancia_usd > 0 else f"-${abs(ganancia_usd):.2f}"
            res_color = "🟢" if ganancia_usd > 0 else "🔴"
            
            md_content += f"| {trade_num} | {t_type} | {t_in} | {t_out} | {exit_reason} | {res_color} **{res_val}** | **${capital:.2f}** |\\n"
    ...
    # Add metadata to trigger artifact creation properly
    meta = '{"ArtifactType":"analysis_results","RequestFeedback":false,"Summary":"Detailed step-by-step graphical log of Solana trades.","UserFacing":true}'
