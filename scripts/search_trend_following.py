...
def prepare_data(df):
    ...
    # MACRO FEATURES (New for Trend Following)
    ...
    df['RET_24'] = df['close'].pct_change(24) # 6 hours
    df['RET_96'] = df['close'].pct_change(96) # 24 hours
    ...
    # TARGET GENERATION (Massive swings)
    target_long, target_short = [], []
    close, high, low, atr = df['close'].values, df['high'].values, df['low'].values, df['ATR'].values
    n = len(df)
    
    max_hold = 192 # 48 hours to find a massive swing
    # Hard thresholds for training labels (We want at least 4.0 ATR profit, max 2.0 ATR loss)
...
def run_backtest(models_long, models_short, start_idx, end_idx, confidence, sl_mult, tp_mult, risk_pct):
    COM, slippage, max_hold = 0.0004, 0.0003, 192 # Up to 48 hours
    ...
    for sym in models_long.keys():
        ...
        while i < n - max_hold:
            ...
            if is_l or is_s:
                ...
                # Trend Following Chandelier-like Exit
                # Give it breathing room, e.g. 1.5 ATR trailing stop only activates after 1 ATR of profit
                activation_dist = cur_atr * 1.0 
                ts_trigger = entrada + activation_dist if es_long else entrada - activation_dist
                ts_activated = False
                
                # We will track highest high / lowest low for Chandelier Exit
                ...
                for j in range(1, max_hold + 1):
                    ...
                    if es_long:
                        ...
                        if ts_activated:
                            # Chandelier Exit: SL moves up with the extreme high minus a loose buffer (e.g. 1.5 ATR)
                            nuevo_sl = extreme_price - (atr[i+j] * 1.5)
...
def objective(trial):
    md = trial.suggest_int('max_depth', 2, 5) # Keep it shallow to avoid noise overfitting
    ...
    tp_mult = trial.suggest_float('tp_mult', 3.0, 8.0)
    risk_pct = trial.suggest_float('risk_pct', 0.05, 0.5) # Risk more since we trade less frequently
    ...
    # We expect fewer trades for trend following, but still need some
...
if __name__ == "__main__":
    ...
    print("\\n=== MEJORES PARÁMETROS TREND FOLLOWING ===")
    print(best)
