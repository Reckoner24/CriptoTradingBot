...
def fetch_data(sym, timeframe, limit=10000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_{limit}.csv"
...
def run_backtest(df, start_idx, end_idx, initial_capital, params):
    ...
    while i < n - max_hold:
        ...
        is_l = (rsi[i] < rsi_os) and (close[i] > ema20[i])
        is_s = (rsi[i] > rsi_ob) and (close[i] < ema20[i])
        ...
        if is_l or is_s:
            ...
            # HARD LIMIT ON POSITION SIZE to avoid 1-trade wipeouts
            if pos_size > capital * 15: pos_size = capital * 15
...
def main():
    print("INICIANDO HIGH-FREQUENCY WFO (5m) PARA 30 DIAS")
    sym = 'SOL/USDT'
    # 30 days * 288 candles/day = 8640 candles
    df_raw = fetch_data(sym, '5m', limit=10000)
    df = prepare_data(df_raw)
    
    CANDLES_PER_DAY = 288
    TOTAL_DAYS = 30
    TRAIN_DAYS = 1 # Ultra-fast memory (24 hours)
    MAX_RISK = 0.15 # Lower risk, higher frequency
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        def objective(trial):
            params = {
                ...
                'ts_act_mult': trial.suggest_float('ts_act_mult', 0.5, 1.5),
                'ts_dist_mult': trial.suggest_float('ts_dist_mult', 0.2, 0.8)
            }
            cap, _, t_count = run_backtest(df, train_start, test_start, 250.0, params)
            if t_count < 3: return -1000 # Force high frequency (at least 3 trades a day)
            return cap
        ...
        new_capital, eq, t_count = run_backtest(df, test_start, test_end, total_capital, p)
    ...
    ax.set_title(f"HFT WFO 5m 30d | Semanal OOS: {weekly_avg*100:.2f}%", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/daily_wfo_5m_equity.png", dpi=100, bbox_inches='tight')
