...
def fetch_data(sym, limit=8000):
    binance = ccxt.binance({'enableRateLimit': True, 'options': {'defaultType': 'future'}})
    ms_por_vela = binance.parse_timeframe('15m') * 1000
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_15m_ML_8000.csv"
    ...
    # Descargar completo si no existe o tiene menos del límite
...
def optimize_window(df, train_start, train_end, val_start, val_end):
    ...
    def objective(trial):
        ...
        cap, trd, w_trd = run_backtest_eval(ml, ms, df, val_start, val_end, conf, sl_mult, tp_mult, risk_pct, 62.5)
        
        if trd < 3: return 0
        win_rate = w_trd / trd
        freq_penalty = min(trd / 8.0, 1.0)
        return cap * (win_rate ** 2) * freq_penalty
        
    study = optuna.create_study(direction='maximize')
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    # Using 30 trials for balance between quality and execution time
...
def main():
    print("INICIANDO MEGA-SIMULACION DAILY WALK-FORWARD (ULTIMOS 30 DIAS)")
    
    DAYS = 30
    CANDLES_PER_DAY = 96
    TRAIN_WINDOW = 3750  # ~39 días de historial de entrenamiento
    VAL_WINDOW = 96 * 3  # 3 días de validación
    
    state_file = 'daily_wfo_state.json'
    ...
    if os.path.exists(state_file):
        try:
            with open(state_file, 'r') as f:
                state = json.load(f)
                start_day = state.get('next_day', 0)
                portfolio_capital = state.get('portfolio_capital', 250.0)
                daily_stats = state.get('daily_stats', [])
            print(f"Estado recuperado. Retomando en el Dia {start_day+1} con ${portfolio_capital:.2f}...")
        except:
            pass

    all_dfs = {}
    for sym in symbols:
        print(f"Sincronizando {sym} (8,000 velas)...")
        df_raw = fetch_data(sym, limit=8000)
        all_dfs[sym] = prepare_data(df_raw)
        
    for day in range(start_day, DAYS):
        print(f"\\n--- Simulando Dia {day+1}/{DAYS} (Hace {DAYS-day} dias) ---")
        
        offset_end = (DAYS - day - 1) * CANDLES_PER_DAY
        ...
        for sym in symbols:
            ...
            # Usar ventana de validación justo antes del test
            val_start = test_start - VAL_WINDOW
            val_end = test_start
            
            # Entrenamiento es antes de la validación
            train_start = val_start - TRAIN_WINDOW
            train_end = val_start
            
            print(f"   [{sym}] Optimizando ({TRAIN_WINDOW} velas de estudio, {VAL_WINDOW} validación)...")
            ...
            if params:
                ...
                # Re-entrenar modelo en el 100% de la historia (train+val) antes del test final del día
                ...
                cap_fin, trds, w_trds = run_backtest_eval(ml, ms, df, test_start, test_end, conf, sl_mult, tp_mult, risk_pct, capital_moneda)
                
                profit = cap_fin - capital_moneda
                day_profit += profit
                print(f"      -> {trds} Trades | Profit: ${profit:.2f}")
            else:
                print(f"      -> Optimización descartada (no halló patrón estable).")
                
        portfolio_capital += day_profit
        daily_stats.append({
            'Dia': day + 1,
            'Hace_Dias': DAYS - day,
            'Beneficio': day_profit,
            'Capital_Acumulado': portfolio_capital
        })
        
        print(f"Fin Dia {day+1}. Capital total de Portafolio: ${portfolio_capital:.2f}")
    ...
    print("\\n=================================================")
    print(f"MEGA-SIMULACION COMPLETADA. CAPITAL FINAL: ${portfolio_capital:.2f}")
    
    import matplotlib
    matplotlib.use('Agg')
    fig, ax = plt.subplots(figsize=(12, 6))
    
    dias = [f"Día {s['Dia']}" for s in daily_stats]
    caps = [s['Capital_Acumulado'] for s in daily_stats]
    
    dias.insert(0, "Inicio")
    caps.insert(0, 250.0)
    
    ax.plot(range(len(caps)), caps, color='#00ff00', linewidth=2)
    ax.fill_between(range(len(caps)), caps, 250, where=(np.array(caps) > 250), color='#00ff00', alpha=0.3)
    ax.fill_between(range(len(caps)), caps, 250, where=(np.array(caps) <= 250), color='#ff0000', alpha=0.3)
    
    ax.set_title("Walk-Forward Diario (Últimos 30 días con Re-Entrenamiento Nocturno)", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/daily_walk_forward_equity.png", dpi=100, bbox_inches='tight')
