...
def fetch_data(sym, timeframe, limit=10000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_ULTIMATE_MONTH_{limit}.csv"
...
def main():
    print("INICIANDO DAILY WFO AGRESIVO Y CORTO - SIMULACION DE 30 DIAS")
    ...
    ax.set_title(f"Daily WFO Dinámico (30 Días) | Semanal OOS: {weekly_avg*100:.2f}%", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/daily_wfo_ultimate_month.png", dpi=100, bbox_inches='tight')
    
    with open(f"{artifact_dir}/daily_wfo_ultimate_month_results.md", "w", encoding='utf-8') as f:
        f.write(f"# Análisis de 30 Días: Daily WFO Reparado\n")
        f.write(f"- Capital Inicial: $250.00\n")
