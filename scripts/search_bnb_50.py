...
def optimize_bnb():
    print(f"\\n========== OPTIMIZANDO BNB/USDT PARA WIN RATE > 50% ==========")
    ...
    def objective(trial):
        ...
        lambd = trial.suggest_float('reg_lambda', 0.0, 10.0)
        
        # Confianza extremada baja permitida para BNB para forzar trades
        conf = trial.suggest_float('confidence', 0.45, 0.60) 
        sl_mult = trial.suggest_float('sl_mult', 0.5, 4.0)
        ...
        win_rate = w_trd / trd
        if win_rate <= 0.50: # Exigimos al menos 50%
            return 0
