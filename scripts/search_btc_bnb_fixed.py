...
def optimize_for_symbol(sym):
    print(f"\\n========== OPTIMIZANDO {sym} PARA FRECUENCIA Y RATIO R/R ==========")
    df = prepared_dfs[sym]
    ...
    def objective(trial):
        ...
        # PARAMETROS CORREGIDOS para evitar el desastre R/R inverso
        conf = trial.suggest_float('confidence', 0.50, 0.65) 
        sl_mult = trial.suggest_float('sl_mult', 0.5, 2.5) # ESTRICTO: No permitir stop loss masivos
        tp_mult = trial.suggest_float('tp_mult', 1.5, 6.0) # Obligar a buscar buenos Take Profits
        ...
        # EXIGIR FRECUENCIA: Minimo 20 trades para que no haga solo 4
        if trd < 20: 
            return 0
            
        win_rate = w_trd / trd
        # EXIGIR WIN RATE ACEPTABLE sin romper la banca
        if win_rate < 0.55:
            return 0
    ...
    study.optimize(objective, n_trials=150) # 150 trials por moneda
...
if __name__ == "__main__":
    ...
    print("\\n\\n================ RESULTADOS FINALES FRECUENCIA + R/R ================ ")
