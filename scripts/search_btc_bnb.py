...
def run_backtest_single(ml, ms, df, start_idx, end_idx, confidence, sl_mult, tp_mult, risk_pct):
    ...
    capital = 62.5 # Capital original por moneda
    total_trades = 0
    winning_trades = 0
...
def optimize_for_symbol(sym):
    print(f"\\n\\n========== OPTIMIZANDO INTENSIVAMENTE {sym} ==========")
    df = prepared_dfs[sym]
    ...
    def objective(trial):
        # Ampliamos radicalmente el espacio de búsqueda para forzar que BTC y BNB encuentren trades
        ...
        # Permitir confianzas mas bajas para forzar mas trades en BTC/BNB que son menos volatiles
        conf = trial.suggest_float('confidence', 0.51, 0.65)
        sl_mult = trial.suggest_float('sl_mult', 0.5, 3.5)
        tp_mult = trial.suggest_float('tp_mult', 1.0, 7.0)
        risk_pct = trial.suggest_float('risk_pct', 0.20, 0.35)
        ...
        # Exigimos al menos 15 trades en el set de validacion para asegurar que habra accion en OOS
        if trd < 15: 
            return 0
    ...
    study.optimize(objective, n_trials=100) # 100 trials por moneda para una busqueda exhaustiva
