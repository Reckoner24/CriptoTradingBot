...
# Parameters currently in generate_notebook.py
params_per_symbol = {
...
def run_analysis_single(sym):
    ...
    capital = 62.5 # Initial capital per coin (250 total / 4)
    ...
    df_eval = test_df
    if len(df_eval) <= max_hold: return None
...
if __name__ == "__main__":
    print("Iniciando analisis de OOS por moneda...")
    for sym in symbols:
        res = run_analysis_single(sym)
        if res:
            print(f"\\n--- {sym} ---")
            print(f"Total Trades: {res['total_trades']}")
            print(f"Win Rate: {res['win_rate']*100:.2f}% ({res['winning_trades']} Wins / {res['losing_trades']} Losses)")
            print(f"Gross Profit: ${res['gross_profit']:.2f}")
            print(f"Gross Loss: ${res['gross_loss']:.2f}")
            print(f"Net Profit: ${res['net_profit']:.2f}")
            print(f"Capital Final: ${res['capital_final']:.2f} (Inicial: $62.50)")
