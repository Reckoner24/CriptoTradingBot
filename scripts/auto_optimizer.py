...
def fetch_data(sym, limit=5000):
    ...
    # Si no existe, descargar completo
    todos = []
...
def run_backtest_single(ml, ms, df, start_idx, end_idx, confidence, sl_mult, tp_mult, risk_pct):
    ...
    return capital, total_trades, winning_trades, max_dd

def optimize_symbol(sym, df):
    print(f"\\n[{sym}] Iniciando auto-optimización dinámica...")
    train_size = int(len(df) * 0.75) # 75% train, 25% val
    train_df = df.iloc[:train_size]
    X = train_df[features]
    yl, ys = train_df['TARGET_L'], train_df['TARGET_S']
    
    val_start = int(len(df) * 0.75)
    val_end = len(df)
    ...
    def objective(trial):
        ...
        cap, trd, w_trd, mdd = run_backtest_single(ml, ms, df, val_start, val_end, conf, sl_mult, tp_mult, risk_pct)
...
def main():
    ...
    for sym in symbols:
        print(f"Descargando datos actualizados para {sym}...")
        df_raw = fetch_data(sym, limit=5000)
        df_prep = prepare_data(df_raw)
        
        params = optimize_symbol(sym, df_prep)
        if params:
            best_params_per_symbol[sym] = params
            print(f"Mejores parámetros {sym}: {params}")
    
    output_path = 'best_params_actualizados.json'
    with open(output_path, 'w') as f:
        json.dump(best_params_per_symbol, f, indent=4)
    print(f"\\nOptimización completada. Parámetros guardados en {output_path}")
