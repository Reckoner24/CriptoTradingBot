...
def fetch_data(sym, timeframe, limit=10000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_DAILYWFO_HYPER_{limit}.csv"
...
def prepare_data(df):
    df = df.copy()
    df['RSI'] = ta.rsi(df['close'], length=7) # RSI rapido para forzar mas trades
    df['ATR'] = ta.atr(df['high'], df['low'], df['close'], length=7)
    df.fillna(0, inplace=True)
    return df

def run_backtest(df, start_idx, end_idx, initial_capital, params):
    COM, slippage, max_hold = 0.0004, 0.0003, 50 # Sostener menos tiempo, scalping rapido
    ...
    while i < n - max_hold:
        ...
        # Eliminamos filtros de tendencia, pura accion de precio en RSI hiper-rapido
        is_l = (rsi[i] < rsi_os)
        is_s = (rsi[i] > rsi_ob)
        ...
        if is_l or is_s:
            ...
            if pos_size > capital * 50: pos_size = capital * 50 # Apalancamiento x50
...
def main():
    ...
    CANDLES_PER_DAY = 96
    TOTAL_DAYS = 30
    TRAIN_DAYS = 7 # Entrenar mas rapido, memoria corta
    MAX_RISK = 0.55 # HIPER RIESGO igual al que funciono al 32%
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        def objective(trial):
            ...
            # PENALIZACION BRUTAL POR NO OPERAR
            # Queremos forzar al bot a encontrar trades rentables si o si.
            if t_count < 3: # Minimo 3 trades en la semana de entrenamiento
                return -1000000 
            
            return cap
            
        study = optuna.create_study(direction='maximize')
        study.optimize(objective, n_trials=50) # Mas intentos para compensar la penalizacion
        p = study.best_params
        ...
        print(f"Día {TOTAL_DAYS - day_offset + 1}/30 ({test_date}): PnL ${profit:+.2f} | Cuenta: ${total_capital:.2f}")

    print(f"\\nCAPITAL FINAL REAL OOS: ${total_capital:.2f}")
    
    total_return_pct = ((total_capital - 250) / 250) * 100
    weekly_avg_return = (total_capital / 250) ** (1/4.28) - 1 if total_capital > 0 else -1
    print(f"RETORNO MENSUAL NETO OOS: {total_return_pct:.2f}%")
    ...
    ax.set_title(f"Optimización Diaria HIPER (30 Días) | Semanal OOS: {weekly_avg_return*100:.2f}%", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/daily_walk_forward_hyper_equity.png", dpi=100, bbox_inches='tight')
    
    with open(f"{artifact_dir}/daily_wfo_hyper_results.md", "w", encoding='utf-8') as f:
        f.write(f"# Resultados Optimización Diaria Forzada\n")
        f.write(f"- Capital Final Real OOS: ${total_capital:.2f}\n")
        f.write(f"**Retorno Semanal Promedio OOS: {weekly_avg_return*100:.2f}%**\n")
