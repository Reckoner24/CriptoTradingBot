...
def fetch_data(sym, timeframe, limit=15000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_GRID_{limit}_20d.csv"
    
    hasta_ms = binance.milliseconds()
    if os.path.exists(cache_file):
        df_cache = pd.read_csv(cache_file, index_col='timestamp', parse_dates=True)
        # Validar frescura, si es muy viejo forzar descarga, si no usar cache
        if not df_cache.empty and len(df_cache) >= limit:
            last_time = df_cache.index[-1]
            if pd.Timestamp.now(tz='UTC').tz_localize(None) - last_time < pd.Timedelta(days=1):
...
def run_optimization_for_sym(sym):
    print(f"\n--- INICIANDO BACKTEST 20 DIAS PARA {sym} ---")
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        print(f"[{sym}] Día {TOTAL_DAYS - day_offset + 1}/{TOTAL_DAYS} ({test_date}): PnL ${profit:+.2f} | Cuenta: ${total_capital:.2f} | Trades: {t_count}")
        
        if total_capital <= 10: break

    print(f"CAPITAL FINAL OOS 20D ({sym}): ${total_capital:.2f}")
    # Semanal promedio (20 dias = 2.85 semanas)
    semanas = 20 / 7
    weekly_avg = (total_capital / 250) ** (1/semanas) - 1 if total_capital > 0 else -1
    print(f"RETORNO SEMANAL PROMEDIO OOS (20D): {weekly_avg*100:.2f}%")
    ...
    ax.set_title(f"Real-World WFO AISLADO 20d ({sym}) | Semanal: {weekly_avg*100:.2f}%", fontsize=16, color='white')
