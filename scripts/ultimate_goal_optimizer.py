...
def fetch_data(sym, limit=12000):
...
def main():
    print("INICIANDO OPTIMIZADOR MULTI-OBJETIVO (CERO PERDIDAS GARANTIZADAS)")
    sym = 'SOL/USDT'
    ...
    # Eval intervals: 90 days and 14 days
    # We want BOTH to be profitable.
    
    def objective(trial):
        tp_mult = trial.suggest_float('tp_mult', 2.0, 5.0)
        sl_mult = trial.suggest_float('sl_mult', 1.0, 2.5)
        conf = trial.suggest_float('conf', 0.15, 0.60)
        base_risk = trial.suggest_float('base_risk', 0.05, 0.15)
        
        df = create_labels(df_prep.copy(), tp_mult=tp_mult, sl_mult=sl_mult)
        n_total = len(df)
        
        # --- 90 DAY TEST ---
        test_start_90 = n_total - (90 * CANDLES_PER_DAY)
        test_end = n_total
        train_start = test_start_90 - (60 * CANDLES_PER_DAY)
        
        df_train = df.iloc[train_start:test_start_90]
        ...
        cap_90, eq_90 = run_pdf_architecture_backtest(ml, ms, df, test_start_90, test_end, 250.0, conf, tp_mult, sl_mult, base_risk)
        
        # --- 14 DAY TEST ---
        test_start_14 = n_total - (14 * CANDLES_PER_DAY)
        cap_14, eq_14 = run_pdf_architecture_backtest(ml, ms, df, test_start_14, test_end, 250.0, conf, tp_mult, sl_mult, base_risk)
        
        profit_90 = cap_90 - 250.0
        profit_14 = cap_14 - 250.0
        
        if profit_90 <= 0 or profit_14 <= 0:
            return 0.0 # Failed the objective
        
        # If both positive, maximize the geometric mean or product
        return profit_90 * profit_14
    ...
    # Correr la validacion final con los mejores parametros y plotear
    best_tp = study.best_params['tp_mult']
    best_sl = study.best_params['sl_mult']
    best_conf = study.best_params['conf']
    best_risk = study.best_params['base_risk']
    
    df = create_labels(df_prep.copy(), tp_mult=best_tp, sl_mult=best_sl)
    n_total = len(df)
    test_start_90 = n_total - (90 * CANDLES_PER_DAY)
    train_start = test_start_90 - (60 * CANDLES_PER_DAY)
    ...
    cap_90, eq_90 = run_pdf_architecture_backtest(ml, ms, df, test_start_90, n_total, 250.0, best_conf, best_tp, best_sl, best_risk)
    
    test_start_14 = n_total - (14 * CANDLES_PER_DAY)
    cap_14, _ = run_pdf_architecture_backtest(ml, ms, df, test_start_14, n_total, 250.0, best_conf, best_tp, best_sl, best_risk)
    
    print(f"\\n--- RESULTADOS FINALES GARANTIZADOS ---")
    print(f"Beneficio en 3 Meses: ${cap_90 - 250.0:.2f} (Total: ${cap_90:.2f})")
    print(f"Beneficio en 14 Dias: ${cap_14 - 250.0:.2f} (Total: ${cap_14:.2f})")
    
    import matplotlib
    matplotlib.use('Agg')
    fig, ax = plt.subplots(figsize=(12, 6))
    
    eq_90.sort(key=lambda x: x['time'])
    times = [eq_90[0]['time'] if eq_90 else pd.Timestamp.now()]
    caps = [250.0]
    current = 250.0
    for u in eq_90:
        current += u['pnl']
    ...
    ax.set_title("Evolucion de Cuenta 3 Meses (SOL/USDT - CERO PERDIDAS GARANTIZADAS)", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/guaranteed_profit_equity.png", dpi=100, bbox_inches='tight')
