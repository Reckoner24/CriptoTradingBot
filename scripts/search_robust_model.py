...
def prepare_data(df):
    ...
    for i in range(n):
        ...
        # Objetivos asimétricos fijos para el entrenamiento
...
def run_backtest(models_long, models_short, start_idx, end_idx, confidence, sl_mult, tp_mult):
    ...
    for sym in symbols:
        ...
        while i < n - max_hold:
            ...
            if is_l or is_s:
                ...
                pos_size = (capitals[sym] * 0.10) / max(riesgo_real_pct, 0.001)
...
def objective(trial):
    ...
    # Validar en el 20% medio (60% al 80%)
    df_len = len(prepared_dfs[symbols[0]])
    val_start = int(df_len * 0.60)
    val_end = int(df_len * 0.80)
    
    final_cap_val = run_backtest(models_long, models_short, val_start, val_end, conf, sl_mult, tp_mult)
    return final_cap_val

if __name__ == "__main__":
    study = optuna.create_study(direction='maximize')
    study.optimize(objective, n_trials=50) # Solo 50 para que acabe rápido, pero suficiente para ver el efecto
    
    best = study.best_params
    print("\\n=== MEJORES PARÁMETROS SIN SESGO ===")
    print(best)
    
    # Ahora evaluar en el TEST SET (80% al 100%) - PRUEBA CIEGA
    print("\\nEntrenando modelo final con mejores parámetros en 80%...")
    models_long, models_short = {}, {}
    for sym in symbols:
        if sym not in prepared_dfs: continue
        df = prepared_dfs[sym]
    ...
    final_cap_test = run_backtest(models_long, models_short, test_start, test_end, best['confidence'], best['sl_mult'], best['tp_mult'])
    
    test_days = (test_end - test_start) / 96 # 96 velas de 15m en un día
    print(f"\\n=== RESULTADO PRUEBA CIEGA (TEST OUT-OF-SAMPLE) ===")
    print(f"Días evaluados: {test_days:.1f}")
