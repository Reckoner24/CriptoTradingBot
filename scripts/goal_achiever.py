...
def fetch_data(sym, timeframe, limit=10000):
    ...
    chunk_size = 1000
    cache_file = f"{cache_dir}/{sym.replace('/', '_')}_{timeframe}_GOAL_{limit}.csv"
...
def run_30_day_wfo(sym, timeframe, train_days, max_risk):
    df_raw = fetch_data(sym, timeframe, limit=15000)
    df = prepare_data(df_raw)
    
    CANDLES_PER_DAY = 96 if timeframe == '15m' else 24 if timeframe == '1h' else 288
    TOTAL_DAYS = 30
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        def objective(trial):
            params = {
                ...
                'risk_pct': trial.suggest_float('risk_pct', max_risk*0.3, max_risk),
        ...
        if total_capital <= 10: break # Bankrupt

    weekly_avg = (total_capital / 250) ** (1/4.28) - 1 if total_capital > 0 else -1
    return total_capital, weekly_avg, all_oos_updates

def main():
    print("INICIANDO BUSQUEDA DEL SANTO GRIAL (30 DIAS >15% OOS)")
    # We test multiple coins and configurations until one hits the target
    configs = [
        ('BTC/USDT', 3, 0.25),
        ('ETH/USDT', 3, 0.25),
        ('BNB/USDT', 3, 0.25),
    ...
    for sym, train_d, risk in configs:
        print(f"Probando {sym} | Train {train_d}d | Riesgo {risk}")
        cap, avg, eq = run_30_day_wfo(sym, '15m', train_d, risk)
        print(f"-> Final: ${cap:.2f} (Semanal: {avg*100:.2f}%)")
        
        if avg > best_avg:
            best_avg = avg
            best_cap = cap
            best_eq = eq
            best_cfg = (sym, train_d, risk)
            
        if avg >= 0.15:
            print("!!! OBJETIVO DE 15% SEMANAL A 30 DIAS ALCANZADO !!!")
            break

    print(f"\\n=================================================")
    print(f"MEJOR CONFIGURACION ENCONTRADA: {best_cfg}")
    ...
    ax.set_title(f"Objetivo 15% Semanal | {best_cfg[0]} | OOS: {best_avg*100:.2f}%", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/goal_achiever_equity.png", dpi=100, bbox_inches='tight')
