...
def prepare_data(df):
    ...
    # MACRO FEATURES (To prevent buying during a crash)
...
def run_backtest(models_long, models_short, start_idx, end_idx, confidence, sl_mult, tp_mult, risk_pct):
    COM, slippage, max_hold = 0.0004, 0.0003, 48
...
def objective(trial):
    ...
    risk_pct = trial.suggest_float('risk_pct', 0.1, 0.5)
