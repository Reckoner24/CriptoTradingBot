...
def run_backtest_martingale(df, start_idx, end_idx, initial_capital, params, current_multiplier=1.0):
    ...
    while i < n - max_hold:
        ...
        if is_l or is_s:
            ...
            if base_risk > 0.90: base_risk = 0.90 # Ultra agresivo
            
            pos_size = (capital * base_risk) / max(riesgo_real_pct, 0.001)
            if pos_size > capital * 60: pos_size = capital * 60
...
def main():
    print("INICIANDO DAILY WFO MARTINGALA EXTREMO (30 DIAS)")
    sym = 'SOL/USDT'
    ...
    TOTAL_DAYS = 30
    TRAIN_DAYS = 3
    MAX_BASE_RISK = 0.25 # Riesgo base muy agresivo
    ...
    for day_offset in range(TOTAL_DAYS, 0, -1):
        ...
        def objective(trial):
            params = {
                ...
                'risk_pct': trial.suggest_float('risk_pct', MAX_BASE_RISK*0.7, MAX_BASE_RISK),
    ...
    ax.set_title(f"Martingala WFO 30d | Semanal OOS: {weekly_avg*100:.2f}%", fontsize=16, color='white')
    ...
    plt.savefig(f"{artifact_dir}/daily_wfo_martingale_hyper_equity.png", dpi=100, bbox_inches='tight')
    
    with open(f"{artifact_dir}/daily_wfo_martingale_hyper_results.md", "w", encoding='utf-8') as f:
        f.write(f"# Análisis de 30 Días: Daily WFO Martingala Extrema\n")
