...
def run_backtest(df, start_idx, end_idx, initial_capital, params):
    ...
    while i < n - max_hold:
        ...
        if is_l or is_s:
            ...
            if pos_size > capital * 25: pos_size = capital * 25 # Cap 25x
...
def run_forced_wfo(df, train_weeks, test_weeks, max_risk):
    ...
    for step in range(WFO_WEEKS, 0, -test_weeks):
...
def main():
    print("INICIANDO BUSQUEDA FORZADA WFO HASTA ENCONTRAR >15% OOS")
    df_raw = fetch_data('SOL/USDT', '15m', limit=15000)
    df = prepare_data(df_raw)
    
    # We will search the WFO MACRO configuration
    best_cap = 0
    best_config = {}
    best_eq = []
    
    # Grid search WFO parameters
    for t_w in [2, 3, 4, 6]:
        for test_w in [1, 2]:
            for m_risk in [0.20, 0.30, 0.40]:
                print(f"Probando WFO config: Train {t_w}w, Test {test_w}w, MaxRisk {m_risk}")
                cap, eq = run_forced_wfo(df, t_w, test_w, m_risk)
                weekly_avg = (cap / 250) ** (1/4) - 1 if cap > 0 else -1
                print(f"-> Capital: ${cap:.2f} (Avg: {weekly_avg*100:.2f}%)")
                
                if cap > best_cap:
                    best_cap = cap
                    best_config = {'train': t_w, 'test': test_w, 'risk': m_risk}
                    best_eq = eq
                    
                if weekly_avg >= 0.15:
                    print("!!! CONFIGURACION GANADORA ENCONTRADA !!!")
                    break
            if best_cap > 250 and ((best_cap / 250) ** (1/4) - 1) >= 0.15: break
        if best_cap > 250 and ((best_cap / 250) ** (1/4) - 1) >= 0.15: break

    weekly_avg = (best_cap / 250) ** (1/4) - 1 if best_cap > 0 else -1
    print(f"\\n=================================================")
    ...
    ax.set_title(f"WFO Forzado ({best_config}) | Semanal OOS: {weekly_avg*100:.2f}%", fontsize=16, color='white')
