...
df_raw = pd.read_csv(f"{cache_dir}/{sym.replace('/', '_')}_15m_10000.csv", index_col='timestamp', parse_dates=True).tail(10000)
...
def optimize_bnb():
    print(f"\\n========== OPTIMIZANDO BNB/USDT PARA WIN RATE > 60% ==========")
    ...
    def objective(trial):
        # Ampliamos a niveles maximos para explorar TODO
        md = trial.suggest_int('max_depth', 2, 6)
        lr = trial.suggest_float('learning_rate', 0.01, 0.25)
        alpha = trial.suggest_float('reg_alpha', 0.0, 15.0)
        lambd = trial.suggest_float('reg_lambda', 0.0, 15.0)
        
        conf = trial.suggest_float('confidence', 0.45, 0.75) 
        sl_mult = trial.suggest_float('sl_mult', 0.5, 6.0)
        ...
        # Reducimos un poco el requisito a 10 trades para permitirle concentrarse mas en Win Rate
        if trd < 10: 
            return 0
            
        win_rate = w_trd / trd
        if win_rate < 0.60: # Exigimos ESTRICTAMENTE 60%
            return 0
            
        return cap * win_rate
        
    study = optuna.create_study(direction='maximize')
    study.optimize(objective, n_trials=300) # 300 trials para barrer absolutamente todas las opciones posibles
...
if __name__ == "__main__":
    ...
    if params:
        print("\\n\\n================ RESULTADO BNB 60% ================")
        print(f"'BNB/USDT': {params}")
    else:
        print("\\n\\nFALLO: Ningún trial alcanzó el 60% de Win Rate con 10 trades mínimos.")
