...
def multi_asset_backtest(dfs, p, slippage=0.0003):
    COM = 0.0004
    CAP = 250.0
    ...
    for sym, df in dfs.items():
        ...
        for k in range(len(idx_all)):
            ...
            # Record trade with generic 1.0 size to calculate compounding later chronologically
    ...
    if not all_trades: return [], [CAP], CAP
    ...
    # Portfolio is split into N slots? No, just trade chronologically using available capital.
    # To be safe, if we hit multiple trades at the same timestamp, we just apply them sequentially.
    pnl_array = []
    
    for i, row in trades_df.iterrows():
        # Fractional Risk Position Sizing
        position_size = min((current_cap * risk_per_trade) / row['sl_pct'], current_cap * 20.0) # Max 20x leverage
...
def objective(trial):
    ...
    pnl, capital_curve, final_cap = multi_asset_backtest(dfs, p)
...
if __name__ == "__main__":
    print("Iniciando Optuna Multi-Asset...")
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    study = optuna.create_study(direction='maximize')
    study.optimize(objective, n_trials=300, show_progress_bar=True)
    
    best = study.best_params
    pnl, cap_c, final = multi_asset_backtest(dfs, best)
    wins = np.sum(pnl > 0)
